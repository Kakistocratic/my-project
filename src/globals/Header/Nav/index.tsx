...
import type { ContactInfo, Header as HeaderType } from '@/payload-types'
        ...
        {navItems.map(({ link }, i) => {
          return link ? (
            <CMSLink
              ...
              {...link}
              appearance="link"
              className={cn('text-xl', textColorClass)}
            />
          ) : null
        })}
          ...
          <>
            {contactData.facebookUrl && (
              <a
                ...
                className={cn('hover:opacity-80 transition-opacity', textColorClass)}
                aria-label="Facebook"
              >
                <svg
